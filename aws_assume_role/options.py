...
class Inclusive(click.Option):
    def __init__(self, *args, **kwargs):
        ...
        kwargs["help"] = (kwargs.get("help", "") + " Must be run with " + ", ".join(self.required_with) + ".").strip()
    ...
    def handle_parse_result(self, ctx, opts, args):
        ...
        for inclusive_opt in self.required_with:
            if inclusive_opt not in opts:
                if current_opt:
                    raise click.UsageError("Illegal usage: '" + str(self.name) + "' must be run with " + str(inclusive_opt) + ".")
                else:
                    self.prompt = None
